...
def review_items(items):
    ...
    for m in items:
        for issue in m['issues']:
            result.append(dict(key=f'BH-REV-{len(result)+1:03d}',manual=m['id'],number=m['number'],title=m['title'],status=m['status'],slug=m['slug'],**issue))
    return result
def head(title,base=''):
...
def footer(base=''):
    return f'<footer>ILLUSTRATIVE — NOT TO SCALE<br>Companion plans control design. Current Companion count sheet controls explicit QB hardware.<br><a href="{base}review/index.html">Human review queue</a> · <a href="{base}photos/index.html">Photo references</a> · <a href="{base}index.html">Contents</a></footer>'
...
def publish_web():
    items=catalog();issues=publish_queue(items);publish_photos()
...
def index_pdf():
    ...
    for page in range(2):
        ...
        c.setFont('Helvetica',9);c.drawString(36,459,'33 manuals / 77 instruction pages. 1 RELEASED / 32 REVIEW REQUIRED / 3 BLOCKED.')
        ...
        c.drawRightString(756,28,f'INDEX v2 / {page+1:02d}');c.showPage()
